...
class Model {
  ...
  async save(doc) {
    console.log(
      "Goin to save " + this.name + " in " + this.collection + " collections"
    );
...
const saveCollection = (collectionName, doc) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!doc._id) {
      ...
      collection = collection.map((_doc) => {
        if (_doc._id == doc._id) {
          return doc;
        } else {
          return _doc;
      ...
      reject(
        `No existe el documento con id ${doc._id} en la coleccion  ${collectionName}.json`
      );
    }
...
const matchByKeys = (obj, query) => {
  ...
  for (const key in query) {
    if (query[key] != obj[key]) match = false;
  }
